...
#环境变量bjqwhd CK格式:备注#xtoken 多号换行
#小程序://社服趣味活动/xEE4MZ4OSjuA0PF
...
def run(x_token):
    try:
        ...
        for i in response.get("data",{}).get("activity",[]):
            ...
            if "进行中" in statusView:
                ...
                for m in range(5):
                    for n in range(20):
                        if n == 0:
                            url = "https://qw-api.bjsfxh.com/api/startAnswer"
                            data = {"id": id}
                        else:
                            url = "https://qw-api.bjsfxh.com/api/getQuestion"
                        ...
                        if "用户未认证或未添加企微" in response.get("message", ""):
                            ...
                            url = "https://qw-api.bjsfxh.com/api/submitAnswer"
                    ...
                    if next:
                        ...
                        url = "https://qw-api.bjsfxh.com/api/submitExam"
                        data = {"id":id,"examId":examId}
                        response = requests.post(url=url, headers=header, json=data).json()
                        #print(f"交卷：{response['message']}")
                        url = "https://qw-api.bjsfxh.com/api/examResult"
                        data = {"id":id,"examId":examId}
                        response = requests.post(url=url, headers=header, json=data).json()
                        #print(f"交卷结果：{response['message']}")
                        url = "https://qw-api.bjsfxh.com/api/lottery"
                        data = {"id":id,"examId":examId}
                        for _ in range(5):
                            response = requests.post(url=url, headers=header, json=data).json()
        ...
        url = 'https://qw-api.bjsfxh.com/api/getUserActivity'
